...
class _BrnPairInfoTableState extends State<BrnPairInfoTable> {
  ...
  late PairInfoTableConfig themeData;

  @override
  void initState() {
    themeData = widget.themeData ?? PairInfoTableConfig();
    themeData =
...
mixin PairInfoPart {
  ...
  PairInfoTableConfig configDefaultThemeData(PairInfoTableConfig? themeData,
      {double? rowDistance, double? itemSpacing}) {
    PairInfoTableConfig defaultThemeData;
    defaultThemeData = themeData ?? PairInfoTableConfig();
    defaultThemeData = defaultThemeData.merge(
        PairInfoTableConfig(rowSpacing: rowDistance, itemSpacing: itemSpacing));
    defaultThemeData = BaseThemeConfig.instance
...
class BrnFollowPairInfo extends StatelessWidget with PairInfoPart {
  ...
  @override
  Widget build(BuildContext context) {
    PairInfoTableConfig defaultThemeConfig = configDefaultThemeData(themeData,
        rowDistance: rowDistance, itemSpacing: itemSpacing);

    return _buildRowWidget(defaultThemeConfig);
  }

  Widget _buildRowWidget(PairInfoTableConfig defaultThemeConfig) {
    int index = -1;
...
class BrnAlignPairInfo extends StatelessWidget with PairInfoPart {
  ...
  Widget _buildAlignWidget(
      PairInfoTableConfig defaultThemeConfig, double maxWith) {
    int index = -1;
    Widget table = Table(
